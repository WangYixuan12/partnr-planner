...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "../../third_party/habitat-lab"))
...
class HabitatViewer:
    ...
    def _handle_camera_rotation(self) -> None:
        ...
        if 'left' in self._mouse_buttons_pressed:
            ...
            self._camera_position = self._camera_target - camera_dir * self._camera_distance
    ...
    def run_blocking(self):
        ...
        print("Controls: Mouse drag to rotate, mouse scroll to zoom, WASD/QE to move, R to reset, ESC to exit")
        ...
        self._root.bind("<Button-4>", self._mouse_callback)      # Linux scroll up (root level)
        self._root.bind("<Button-5>", self._mouse_callback)      # Linux scroll down (root level)
        ...
        def update():
            ...
            try:
                ...
                # self._sim.get_agent(self._agent_idx).set_state(
                #     habitat_sim.AgentState(
                #         position=self._camera_position,
                #         rotation=quat_from_magnum(mn.Quaternion.from_matrix(camera_transform.rotation_scaling()))
                #     )
                # )
    ...
    def start_thread(self):
            ...
            try:
                ...
                def update():
                    ...
                    try:
                        ...
                        self._sim._sensors[self._camera_name]._sensor_object.node.translation = trans
                        self._sim._sensors[self._camera_name]._sensor_object.node.rotation = rot
                        self._sim._sensors[self._camera_name]._sensor_object.node.transformation = mn.Matrix4.from_(rot.to_matrix(), trans)
        ...
        print(f"Started Habitat-Sim Viewer in separate thread")
